...
class SkipPathTests(unittest.TestCase):
    ...
    def test_private_coordination_paths_are_not_skipped(self):
        for p in [
            "notes/HANDOFF.md",
    ...
    def test_runtime_paths_are_skipped_at_path_filter_level(self):
        for p in [
...
class ForbiddenPathTests(unittest.TestCase):
    ...
    def test_private_coordination_file_path_is_caught_even_when_empty(self):
        check_leaks.get_file_content_lines = _FakeFs({"notes/HANDOFF.md": ""})
        leaks = check_leaks.scan(["notes/HANDOFF.md"], [], staged=False)
        self.assertEqual(len(leaks), 1)
        self.assertEqual(leaks[0].pattern_label, "private coordination path")
        self.assertEqual(leaks[0].line_no, 0)

    def test_public_doc_path_is_allowed(self):
        check_leaks.get_file_content_lines = _FakeFs({"docs/SCHEMAS.md": ""})
        leaks = check_leaks.scan(["docs/SCHEMAS.md"], [], staged=False)
        self.assertEqual(leaks, [])
