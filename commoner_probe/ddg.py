...
"""Deprecated name for :mod:`commoner_probe.ministry_pdf_index`.
...
from . import ministry_pdf_index as _target

_warnings.warn(
    "commoner_probe.ddg is deprecated; import commoner_probe.ministry_pdf_index instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.ddg import X`,
# `commoner_probe.ddg.X` and `monkeypatch.setattr("commoner_probe.ddg.X", ...)`
