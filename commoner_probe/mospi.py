...
"""Deprecated name for :mod:`commoner_probe.rest_dataset_api`.
...
from . import rest_dataset_api as _target

_warnings.warn(
    "commoner_probe.mospi is deprecated; import commoner_probe.rest_dataset_api instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.mospi import X`,
# `commoner_probe.mospi.X` and `monkeypatch.setattr("commoner_probe.mospi.X", ...)`
