...
"""Deprecated name for :mod:`commoner_probe.verbatim_pdf_api`.
...
from . import verbatim_pdf_api as _target

_warnings.warn(
    "commoner_probe.debates is deprecated; import commoner_probe.verbatim_pdf_api instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.debates import X`,
# `commoner_probe.debates.X` and `monkeypatch.setattr("commoner_probe.debates.X", ...)`
