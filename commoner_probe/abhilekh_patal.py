...
"""Deprecated name for :mod:`commoner_probe.catalogue_search_api`.
...
from . import catalogue_search_api as _target

_warnings.warn(
    "commoner_probe.abhilekh_patal is deprecated; import commoner_probe.catalogue_search_api instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.abhilekh_patal import X`,
# `commoner_probe.abhilekh_patal.X` and `monkeypatch.setattr("commoner_probe.abhilekh_patal.X", ...)`
