...
"""Deprecated name for :mod:`commoner_probe.audit_pdf_index`.
...
from . import audit_pdf_index as _target

_warnings.warn(
    "commoner_probe.cag is deprecated; import commoner_probe.audit_pdf_index instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.cag import X`,
# `commoner_probe.cag.X` and `monkeypatch.setattr("commoner_probe.cag.X", ...)`
