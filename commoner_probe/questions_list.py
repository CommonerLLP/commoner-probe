...
"""Deprecated name for :mod:`commoner_probe.question_list_api`.
...
from . import question_list_api as _target

_warnings.warn(
    "commoner_probe.questions_list is deprecated; import commoner_probe.question_list_api instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.questions_list import X`,
# `commoner_probe.questions_list.X` and `monkeypatch.setattr("commoner_probe.questions_list.X", ...)`
