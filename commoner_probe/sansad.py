...
"""Deprecated name for :mod:`commoner_probe.parliament_qa_api`.
...
from . import parliament_qa_api as _target

_warnings.warn(
    "commoner_probe.sansad is deprecated; import commoner_probe.parliament_qa_api instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.sansad import X`,
# `commoner_probe.sansad.X` and `monkeypatch.setattr("commoner_probe.sansad.X", ...)`
