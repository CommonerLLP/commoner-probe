...
"""Deprecated name for :mod:`commoner_probe.committee_report_api`.
...
from . import committee_report_api as _target

_warnings.warn(
    "commoner_probe.committees is deprecated; import commoner_probe.committee_report_api instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.committees import X`,
# `commoner_probe.committees.X` and `monkeypatch.setattr("commoner_probe.committees.X", ...)`
