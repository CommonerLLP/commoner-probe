...
"""Deprecated name for :mod:`commoner_probe.drupal_publication_index`.
...
from . import drupal_publication_index as _target

_warnings.warn(
    "commoner_probe.prs is deprecated; import commoner_probe.drupal_publication_index instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.prs import X`,
# `commoner_probe.prs.X` and `monkeypatch.setattr("commoner_probe.prs.X", ...)`
