...
"""Deprecated name for :mod:`commoner_probe.affidavit_pages`.
...
from . import affidavit_pages as _target

_warnings.warn(
    "commoner_probe.myneta is deprecated; import commoner_probe.affidavit_pages instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.myneta import X`,
# `commoner_probe.myneta.X` and `monkeypatch.setattr("commoner_probe.myneta.X", ...)`
