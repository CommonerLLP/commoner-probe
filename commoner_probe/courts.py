...
"""Deprecated name for :mod:`commoner_probe.case_law_api`.
...
from . import case_law_api as _target

_warnings.warn(
    "commoner_probe.courts is deprecated; import commoner_probe.case_law_api instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.courts import X`,
# `commoner_probe.courts.X` and `monkeypatch.setattr("commoner_probe.courts.X", ...)`
