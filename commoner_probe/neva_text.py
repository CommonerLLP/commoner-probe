...
"""Deprecated name for :mod:`commoner_probe.two_column_pdf_qa`.
...
from . import two_column_pdf_qa as _target

_warnings.warn(
    "commoner_probe.neva_text is deprecated; import commoner_probe.two_column_pdf_qa instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.neva_text import X`,
# `commoner_probe.neva_text.X` and `monkeypatch.setattr("commoner_probe.neva_text.X", ...)`
