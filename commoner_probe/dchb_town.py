...
"""Deprecated name for :mod:`commoner_probe.ogd_town_release`.
...
from . import ogd_town_release as _target

_warnings.warn(
    "commoner_probe.dchb_town is deprecated; import commoner_probe.ogd_town_release instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.dchb_town import X`,
# `commoner_probe.dchb_town.X` and `monkeypatch.setattr("commoner_probe.dchb_town.X", ...)`
