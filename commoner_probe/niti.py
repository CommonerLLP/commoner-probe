...
"""Deprecated name for :mod:`commoner_probe.annual_report_index`.
...
from . import annual_report_index as _target

_warnings.warn(
    "commoner_probe.niti is deprecated; import commoner_probe.annual_report_index instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.niti import X`,
# `commoner_probe.niti.X` and `monkeypatch.setattr("commoner_probe.niti.X", ...)`
