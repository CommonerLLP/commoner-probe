...
"""Deprecated name for :mod:`commoner_probe.attendance_json_api`.
...
from . import attendance_json_api as _target

_warnings.warn(
    "commoner_probe.attendance is deprecated; import commoner_probe.attendance_json_api instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.attendance import X`,
# `commoner_probe.attendance.X` and `monkeypatch.setattr("commoner_probe.attendance.X", ...)`
