...
"""Deprecated name for :mod:`commoner_probe.bill_catalog_api`.
...
from . import bill_catalog_api as _target

_warnings.warn(
    "commoner_probe.bills is deprecated; import commoner_probe.bill_catalog_api instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.bills import X`,
# `commoner_probe.bills.X` and `monkeypatch.setattr("commoner_probe.bills.X", ...)`
