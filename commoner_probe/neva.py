...
"""Deprecated name for :mod:`commoner_probe.assembly_portal`.
...
from . import assembly_portal as _target

_warnings.warn(
    "commoner_probe.neva is deprecated; import commoner_probe.assembly_portal instead. "
    "The new name states the mechanism rather than the scheme.",
    DeprecationWarning,
    stacklevel=2,
)

# Bind the target module itself, so `from commoner_probe.neva import X`,
# `commoner_probe.neva.X` and `monkeypatch.setattr("commoner_probe.neva.X", ...)`
