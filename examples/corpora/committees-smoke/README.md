...
Frozen smoke fixture for the standing-committee probe.
...
The live smoke (running the CLI against sansad.in) proves the probe
...
probe fails but this fixture still passes, the upstream API changed.

## Refreshing

The frozen payloads should change rarely — only when the upstream API
shape changes in a way the probe must now handle. Refresh procedure:
