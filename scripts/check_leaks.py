...
1. **Public patterns** — hardcoded below. Uncontroversial structural
   ...
   tracked file: forbidden coordination paths, cross-references to private
   local notes, local filesystem paths, internal corpus tags that have
   already been exposed historically (``ADP``), and internal
   product-vocabulary terms that read as a tell.
...
Skipped files (always): the script itself, anything under ``data/``, the
``.git`` tree, local virtualenv/build output, and binary fixtures.
"""
...
PUBLIC_PATTERNS: list[tuple[str, str]] = [
    (r"/Users/[A-Za-z0-9._-]+", "local absolute path"),
    (r"\b[A-Za-z0-9._%+-]+@NH-[A-Za-z0-9._-]+\.local\b", "local machine email address"),
    (r"\bssh\b.*\b-i\s+~?/\.?ssh/", "ssh private-key command"),
    (r"\b[A-Za-z0-9._-]+@[0-9]{1,3}(?:\.[0-9]{1,3}){3}\b", "ssh user/host target"),
    ...
    (r"\bnotes/(?:ROADMAP|PRODUCT_DESIGN|TECHDEBT|PRINCIPLES|INTERNAL_CHANGELOG|STATE_OF_BRAIN|state-of-brain|handoffs)\b",
...
FORBIDDEN_TRACKED_PATHS: list[tuple[re.Pattern[str], str]] = [
    (re.compile(r"^(?:notes|memory|\.ai|\.claude|\.codex)(?:/|$)"), "private coordination path"),
    (re.compile(r"^\.beads/"), "beads internal path"),
    (re.compile(r"^(?:WORKING|TODO)\.md$"), "private coordination file"),
    (re.compile(r"^docs/(?:STATUS|TODO-[^/]+)\.md$"), "private status/planning doc"),
]
...
def scan(
    ...
    *,
    ...
    leaks: list[Leak] = []
    for path in files:
        for pat, label in FORBIDDEN_TRACKED_PATHS:
            if pat.search(path):
                leaks.append(Leak(
                    path=path,
                    line_no=0,
